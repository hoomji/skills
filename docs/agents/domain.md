...
If any of these files don't exist, **proceed silently**. Don't flag their absence or create them upfront. `/mattpocock-skills:domain-modeling` resolves vocabulary and consequential trade-offs; `harness-model-domain` maintains the glossary lazily, while `harness-record-decision` records and manages a qualifying ADR.
